...
auto get_tuple_of_size(std::integer_sequence<IntType, Idxs...>)
    -> decltype(std::make_tuple((Idxs, std::declval<T>())...)) {
  return {};
}
...
using XSumSolution = decltype(detail::get_tuple_of_size<T,N>()) ;
...
template <typename... T>
inline std::ostream& operator<<(std::ostream& os, const std::optional<std::tuple<T...>>& solution) {
...
template <std::size_t X> struct XSum {
  ...
  [[nodiscard]] auto find(VALUE_TYPE &&target, CONTAINER container) const
      -> std::optional<XSumSolution<VALUE_TYPE, X>> {
...
template <> struct XSum<2> {
  template <typename CONTAINER,
            typename VALUE_TYPE = typename CONTAINER::value_type>
  [[nodiscard]] auto find(VALUE_TYPE &&target, CONTAINER container) const
      -> std::optional<XSumSolution<VALUE_TYPE, 2>> {
    ...
    for (std::size_t i = 0; i < container.size(); ++i) {
      ...
      if (foundIt != map.end()) {
        return std::make_optional(
            std::make_tuple(container.at(i), foundIt->first));
      }
...
template <> struct XSum<0> {
  ...
  template <typename CONTAINER,
            typename VALUE_TYPE = typename CONTAINER::value_type>
  auto find(int target, CONTAINER &&numbers) {
    static_assert(sizeof(CONTAINER) == 0,
                  "XSum only accepts values of 2 or more.");
  }
};
